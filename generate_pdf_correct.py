...
for day in range(6):
    ...
    if reading_data:
        ...
        if not passages:
            # ربما الملف منظم كمصفوفة مباشرة
        ...
        for pi, passage in enumerate(passages):
            ...
            if questions:
                pdf.set_font('Amiri', 'B', 11)
                pdf.cell(0, 8, 'أسئلة الفهم:', ln=True)
                pdf.set_font('Amiri', '', 10)
    ...
    if vocab_data:
        ...
        if questions:
            pdf.set_font('Amiri', 'B', 11)
            pdf.cell(0, 8, 'أسئلة المفردات:', ln=True)
            pdf.set_font('Amiri', '', 10)
            print_questions(pdf, questions, 10)
        pdf.add_line()
    ...
    if dict_data:
        ...
        if not words and isinstance(dict_data, list):
            words = dict_data
        for w in words:
            correct = w.get('correct', '') if isinstance(w, dict) else w
            options = w.get('options', []) if isinstance(w, dict) else []
    ...
    if gramtest_data:
        ...
        if isinstance(gramtest_data, list):
            print_questions(pdf, gramtest_data, 10)
        else:
            questions = gramtest_data.get('questions', [])
            if questions:
                print_questions(pdf, questions, 10)
        pdf.add_line()
    ...
    if hw_data:
        ...
        # حسب الهيكل: يمكن أن يكون مصفوفة من passages مع أسئلة
...
for w in all_words:
    ...
    if eng not in seen:
        ...
        pdf.cell(col_w_v[1], 7, eng, border=1)
        pdf.cell(col_w_v[2], 7, pdf.reshaped(w.get('arabic','')), border=1)
        pdf.ln()
...
print("✅ تم إنشاء PDF الكامل من مجلدات day-XX")
