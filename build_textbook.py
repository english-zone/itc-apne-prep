        ...
        if questions:
            ...
            for q in questions:
                ...
                for j, opt in enumerate(opts):
                    correct = " correct" if j == ans else ""
                    sec += f'<li class="{correct}">{escape(opt)}</li>\n'
                sec += '</ol></li>\n'
            sec += '</ol></div>\n'
        sec += '</article>\n'
    sec += '</section>\n'
...
def build_vocab(day_num):
    ...
    if not questions and isinstance(v, list):
        questions = v
    ...
    if words:
        ...
        for i, w in enumerate(words):
            sec += f'<tr><td>{i+1}</td><td class="en">{escape(w.get("english",""))}</td><td class="ar">{escape(w.get("arabic",""))}</td></tr>\n'
        sec += '</tbody></table>\n'
    if questions:
...
def build_answer_keys():
    sec = '<section class="answer-keys"><h2 class="section-title">مفاتيح الإجابات</h2>'
    for d in range(1, 8):
        g = load_json(f"content/day-{d:02d}/grammar-test.json")
        if g:
            questions = g.get("questions", []) if isinstance(g, dict) else g
            if questions:
                sec += f'<h3>اليوم {d} – اختبار القواعد</h3><ol>'
                for q in questions:
                    q_text = escape(q.get("q", ""))
                    opts = q.get("options", [])
                    ans = q.get("answer", -1)
                    correct = opts[ans] if 0 <= ans < len(opts) else "—"
                    sec += f'<li>{q_text} → <strong>{escape(correct)}</strong></li>'
                sec += '</ol>'
    for i in range(1, 7):
        c = load_json(f"content/exams/compilation-{i}.json")
        if c:
...
.answer-keys { margin-top: 4rem; }
.answer-keys h3 { margin-top: 1.5rem; color: var(--primary); }
...
html += '<p>الحمد لله الذي علّم بالقلم… (النص الكامل للمقدمة كما كتبتَه سابقاً). أسأل الله أن ينفع بهذا العمل.</p>'
