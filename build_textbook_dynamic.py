...
def get_reading_title(day_num):
    ...
    if not passages and isinstance(r, list):
        passages = r
    if passages:
        first_passage = passages[0] if isinstance(passages[0], dict) else None
        if first_passage:
            return first_passage.get("title", f"Passage {day_num}")
    return f"Day {day_num} Reading"
...
def get_grammar_title(day_num):
    # يمكن تحسينه لاحقاً بقراءة عنوان من ملف HTML أو اسم الملف
    titles = ["Present/Past Simple", "Continuous Tenses", "Perfect Tenses",
              "Passive Voice", "Comparative/Superlative", "Gerund/Infinitive", "مراجعة شاملة"]
    return titles[day_num-1] if day_num <= 7 else "Grammar"
...
for d in range(1, 8):
    ...
    grammar_title = get_grammar_title(d)
    syllabus_rows += f'<tr><td>اليوم {d}</td><td>{escape(reading_title)}</td><td>{escape(vocab_topic)}</td><td>{escape(grammar_title)}</td></tr>\n'

day_titles_dynamic = [get_reading_title(d) for d in range(1, 8)]
...
# أيام
for d in range(1, 8):
    html += f'<div class="day" id="day-{d}"><h1 class="section-title">اليوم {d} – {day_titles_dynamic[d-1]}</h1>'
