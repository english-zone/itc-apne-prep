import json, os, html as html_mod, itertools
...
def get_passages(day_num):
    ...
    for p in passages:
        ...
        if title not in seen:
            seen.add(title)
            unique.append(p)
    # اقتصاص على أول 3 قطع
    return unique[:3]

def build_reading(day_num):
    passages = get_passages(day_num)
    if not passages:
        return ""
...
def build_vocab(day_num, limit_questions=20):
    ...
    for i, w in enumerate(words):
        ...
        table_rows += f'<tr><td>{i+1}</td><td class="en" dir="ltr">{eng}</td><td class="ar">{ara}</td></tr>\n'
    ...
    all_questions = all_questions[:limit_questions]
    questions_html = ""
    if all_questions:
    ...
    if questions_html:
        ...
        sec = f'<section class="vocabulary">\n<h2 class="section-title">المفردات</h2>\n{table}\n</section>\n'
    return sec
...
def build_grammar_test(day_num, limit=20):
...
def get_reading_title(day_num):
    ...
    if passages and isinstance(passages[0], dict):
        return passages[0].get("title", f"Passage {day_num}")
...
intro_text = """(المقدمة الكاملة هنا)"""
...
<title>دليل الطالب للتميز في APNE‑ITC</title>
<style>
...
table.word-table td, table.word-table th { border: 1px solid #ccc; padding: 6px 8px; }
.en { font-family: 'Inter', sans-serif; }
[dir="ltr"] { direction: ltr; unicode-bidi: embed; }
.vocab-flex { display: flex; gap: 2rem; flex-wrap: wrap; }
.vocab-col { flex: 1 1 45%; }
@media (max-width: 768px) {
  .vocab-flex { flex-direction: column; }
  .vocab-col { flex: 1 1 100%; }
...
for d in range(1, 8):
    ...
    html += build_vocab(d)  # 20 سؤال افتراضي
    html += build_dictation(d)
    html += build_grammar_lesson(d)
    html += build_grammar_test(d)  # 20 سؤال افتراضي
    html += '</div>'
